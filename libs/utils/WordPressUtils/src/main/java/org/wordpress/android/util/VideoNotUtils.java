 ...
 *
 * <p>It uses MediaExtractor to get frames from a test stream, decodes them to a surface,
 * encodes them from the resulting surface, and then uses MediaMuxer to write
 * them into a file.
 *
 ...
 */
@TargetApi(18)
public class VideoNotUtils {
    private static final String TAG = VideoNotUtils.class.getSimpleName();
    ...
    public VideoNotUtils(final Context context, final Uri videoUri) {
        new Thread() {
            @Override
            public void run() {
                try {
                    ProfilingUtils.start("Video Encoding Test");
                    ...
                    setOutputFile();
                    try {
                        extractDecodeEditEncodeMux(context, videoUri);
                    } catch (Throwable th) {
                        AppLog.e(AppLog.T.MEDIA, "Grave error running extractDecodeEditEncodeMux", th);
                    }
                    ProfilingUtils.split("176x144 done");
                    ...
                    ProfilingUtils.split("320x240 done");
                    ...
                    ProfilingUtils.split("1024x768 done");
     ...
     */
    private void extractDecodeEditEncodeMux(Context context, Uri videoUri) throws Exception {
        ...
        try {
            if (mCopyVideo) {
                videoExtractor = createExtractor(context, videoUri);
                int videoInputTrack = getAndSelectVideoTrackIndex(videoExtractor);
                if (videoInputTrack == -1) {
                    Log.e(TAG, "missing video track in test video");
                    return;
                }
                ...
                //int degrees = VideoUtils.getVideoRotationDegrees(context, videoUri);
                //outputVideoFormat.setInteger(MediaFormat.KEY_ROTATION, degrees);
            ...
            if (mCopyAudio) {
                audioExtractor = createExtractor(context, videoUri);
    ...
    /**
     * Creates an extractor that reads the file
     */
    private MediaExtractor createExtractor(Context context, Uri videoUri) throws IOException {
    ...
    private void doExtractDecodeEditEncodeMux(
        ...
        if (mCopyVideo) {
            if ( videoDecodedFrameCount != videoEncodedFrameCount ) {
                Log.e(TAG, "encoded and decoded video frame counts does NOT match");
                return;
            }
        ...
        if (mCopyAudio) {
            if ( -1 != pendingAudioDecoderOutputBufferIndex ) {
                Log.e(TAG, "no audioframe should be pending");
            }
        }
